...
def test_live_training_parity(data, season, week):
    ...
    t = _target(g, gid)
    feats, status = build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
...
def test_target_score_invariance(data):
    ...
    t = _target(g, gid)
    a, _ = build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
    g2 = g.copy()
    m = g2["game_id"].astype(str) == gid
    g2.loc[m, "home_score"] = 999  # corrupt the target's own result
    g2.loc[m, "away_score"] = 0
    b, _ = build_live_augmented_features(g2, t, pbp, as_of_utc=_asof_before(t))
    for c in FROZEN_FEATURES:
...
def test_target_pbp_invariance(data):
    ...
    b, _ = build_live_augmented_features(g, t, pbp2, as_of_utc=_asof_before(t))
    for c in FROZEN_FEATURES:
        if pd.notna(a[c].iloc[0]):
            assert abs(float(a[c].iloc[0]) - float(b[c].iloc[0])) < 1e-12
...
def test_target_kickoff_at_or_before_asof_fails(data):
    ...
    with pytest.raises(ValueError, match="at or before"):
        build_live_augmented_features(g, t, pbp, as_of_utc=asof)
...
def test_duplicate_target_fails(data):
    ...
    with pytest.raises(ValueError, match="duplicate target"):
        build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
...
def test_post_asof_game_excluded_changes_nothing_earlier(data):
    ...
    t = _target(g, early)
    a, sa = build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
...
def test_deterministic_hash(data):
    ...
    gid = str(matrix[(matrix["season"] == 2024) & (matrix["week"] == 10)].head(1)["game_id"].iloc[0])
    t = _target(g, gid)
    _, s1 = build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
    _, s2 = build_live_augmented_features(g, t, pbp, as_of_utc=_asof_before(t))
