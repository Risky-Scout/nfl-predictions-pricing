...
Fix 6 (Week 1 prior + compact feature deduction) is in progress on
`fix/week1-prior-feature-deduction`; when it merges, update its row to
`MERGED`, add the PR reference, and re-check whether "Model-family /
frozen-spec selection" can move off `PENDING` (it depends on Fix 6's frozen
feature set but is itself a separate, not-yet-started stage).
